...
VULCAN_YML_BUILD_SUBDIR = os.getenv("VULCAN_YML_BUILD_SUBDIR")
...
def run_apr():
    ...
    if VULCAN_YML_BUILD_SUBDIR == '':
        msv_runner_cmd = f"python3 {MSV_REPO}/msv-runner.py -s {MUTABLE_ENV['FL_JSON']} -r {VULCAN_TARGET} {MUTABLE_ENV['MSV_WORKSPACE']} {MSV_REPO}"
    else:
        msv_runner_cmd = f"python3 {MSV_REPO}/msv-runner.py -s {MUTABLE_ENV['FL_JSON']} -d {VULCAN_YML_BUILD_SUBDIR} -r {VULCAN_TARGET} {MUTABLE_ENV['MSV_WORKSPACE']} {MSV_REPO}"
