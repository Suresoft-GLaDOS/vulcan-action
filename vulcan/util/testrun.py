...
def _clean_after_collect_gcov():
    gcov_exclusion_list = VULCAN_YML_TEST_LIST.splitlines() if VULCAN_YML_TEST_LIST is not None else ""
    gcov_exclusion_list_command = "-e ".join(gcov_exclusion_list)
    print("gcov_exclusion_list_command = ", gcov_exclusion_list_command)
