...
def _clean_after_collect_gcov():
    ...
    gcov_inclusion_list_command = ''.join([' -i ' + e for e in gcov_inclusion_list])
    print(f"python3 {GITHUB_ACTION_PATH}/vulcan/util/gcovg.py -r {VULCAN_TARGET} -f \"*.o\" -o {VULCAN_OUTPUT_DIR}/gcov_map.json {gcov_exclusion_list_command} {gcov_inclusion_list_command}")
    os.system(f"python3 {GITHUB_ACTION_PATH}/vulcan/util/gcovg.py -r {VULCAN_TARGET} -f \"*.o\" -o {VULCAN_OUTPUT_DIR}/gcov_map.json {gcov_exclusion_list_command} {gcov_inclusion_list_command}")
